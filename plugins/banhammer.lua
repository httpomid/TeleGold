...
--vardump(extra)
      ...
      elseif get_cmd == 'بن' then
        ...
        send_large_msg(receiver, '🔱کاربر @'..member..' ['..member_id..'] از این گروه مسدود شد. 🔱')
      ...
      elseif get_cmd == 'سوپر بن' then
        send_large_msg(receiver, '🔱 کاربر @'..member..' ['..member_id..'] از همه گروهای ربات تله گولد بصورت دائم و جهانی حذف شد. 🔱')
        return banall_user(member_id, chat_id)
      elseif get_cmd == 'حذف سوپر بن' then
        send_large_msg(receiver, '🔱 کاربر @'..member..' ['..member_id..'] از سوپر  بن خارج شد و اجازه ورود به گروه های ربات تله گولد را دارد. 🔱')
  ...
  if matches[1]:lower() == 'حذف بن' or matches[1]:lower() == 'unban' then -- /unban 
    if type(msg.reply_id)~="nil" and is_momod(msg) then
      ...
      if string.match(targetuser, '^%d+$') then
        	...
        	return '🔱 کاربر: '..user_id..' آن بن شد و اجازه ورود به گروه را دارد. 🔱'
      else
		local cbres_extra = {
			chat_id = msg.to.id,
			get_cmd = 'حذف بن',
      ...
      if string.match(targetuser, '^%d+$') then
        if tonumber(matches[2]) == tonumber(our_id) then
        	...
        	banall_user(targetuser)
       		return '🔱کاربر ['..user_id..' ] از همه ی گروه ها بصورت دائمی و جهانی حذف شد. 🔱'
      else
	local cbres_extra = {
		chat_id = msg.to.id,
		get_cmd = 'سوپر بن',
    ...
    local chat_id = msg.to.id
      if string.match(matches[2], '^%d+$') then
        if tonumber(matches[2]) == tonumber(our_id) then 
        	...
        	return '🔱کاربر ['..user_id..' ] از لیست سوپر بن ربات تله گولد خارج شد و اجازه ی ورود به گروه را دارد. 🔱'
      else
	local cbres_extra = {
		chat_id = msg.to.id,
		get_cmd = 'حذف سوپر بن',
...
-plugins by @SoftPlugin is Poen @SoftTG_TEAM Or Team Soft
