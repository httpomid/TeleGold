		...
		if matches[1] == 'mute' and is_owner(msg) then
			...
			local msg_type = 'All'
				if not is_muted(chat_id, msg_type..': yes') then
					savelog(msg.to.id, name_log.." ["..msg.from.id.."] set SuperGroup to: mute "..msg_type)
					mute(chat_id, msg_type)
					return "🔱 "..msg_type.." گروه در حالت چت ممنوع قرار گرفت. 🔱 "
				else
					return "🔱 "..msg_type.." چت از قبل ممنوع بود.🔱"
				end
			end
		end
		if matches[1] == 'unmute' and is_momod(msg) then
			...
			local msg_type = 'All'
				if is_muted(chat_id, msg_type..': yes') then
					...
					return "🔱 "..msg_type.." چت از قبل آزاد بود.🔱"
				end
			end
		end
...
برای مقام دادن به شخصی ریپلی #praytime 
برای حذف مدیریت آیدی فرد را جلوی #demote بزارید.
برای تعویض نام جلوی #setname نام جدید را بنویسید.
...
برای بیصدا کردن این موارد links-flood-spam-arabic-member-rt-sticker-contects-strict-tgservice-forward کافیه یکی از این موارد را جلوی #lock قرار دهید و برای فعال کردن یکی از موارد را جلوی #unlock بزارید.
